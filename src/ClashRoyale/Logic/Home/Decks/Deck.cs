...
namespace ClashRoyale.Logic.Home.Decks
{
    public class Deck : List<Card>
    {
        ...
        public void Encode(IByteBuffer packet)
        {
            ...
            foreach (Card deckcard in Home.Deck)
            {
                bool found = false; 
                foreach (Card card in Cards.GetAllCards())
                {
                    if (deckcard.ClassId == card.ClassId && deckcard.InstanceId == card.InstanceId)
                    {
                        found = true;
                        break;
                    }
		    ...
		    for (int i = 0; i < 8; i++)
	            {
	               Home.Deck.Add(new Card(26, Cards.GetAllCards()[0].InstanceId + i, false));
	            }
	            for (int i = 0; i < 6; i++)
		    {
                        Home.Decks[i] = new int[] {Home.Deck[0].GlobalId, Home.Deck[1].GlobalId, Home.Deck[2].GlobalId, Home.Deck[3].GlobalId, Home.Deck[4].GlobalId, Home.Deck[5].GlobalId, Home.Deck[6].GlobalId, Home.Deck[7].GlobalId};
                    }
	            break;
                }
            }
            ...
            packet.WriteVInt((Home.SelectedDeck < 5 ? Home.SelectedDeck : 4)); // CurrentSlot
        }
        ...
        /// <summary>
        /// Encodes this deck for a mindeck battle
        /// </summary>
        /// <param name="packet"></param>
        /// <param name="deckcardcount">Only The (deckcardcount) first cards will be encoded</param>
        public void EncodeMinideckAttack(IByteBuffer packet, int deckcardcount)
        {
            foreach (var card in GetRange(0, 8))
            {
            	if (deckcardcount > 0)
                ...
                {
                    (new Card(26, 0, false)).EncodeAttack(packet);
                }
            }
         }
        ...
        public void SwapCardTournament(int cardOffset, int deckOffset)
        {
            var currentDeck = Home.Decks[5];
            currentDeck[deckOffset] = Cards.GetAllCards()[cardOffset].GlobalId;
        }
        /// <summary>
