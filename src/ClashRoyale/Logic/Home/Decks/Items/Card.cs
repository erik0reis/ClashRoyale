        ...
        public void EncodeAttack(IByteBuffer packet)
        {
            try
            {
                if (ClassId == 26)
                {
                    var data = Csv.Tables.Get(Csv.Files.SpellsCharacters)
                                .GetDataWithInstanceId<SpellsCharacters>(InstanceId);
                    if (data != null && data.Name.Contains("="))
                    {
                        var CardToReplace = data.Name.Split("=");
                        var CardToReplaceClassId = int.Parse(CardToReplace[0]);
                        var CardToReplaceInstanceId = int.Parse(CardToReplace[1]);
                        var CardToReplaceId = Id(CardToReplaceClassId, CardToReplaceInstanceId);
                        packet.WriteVInt(CardToReplaceId);
                    }
                    else
                    {
                        packet.WriteVInt(CardId);
