...
namespace ClashRoyale.Logic.Home.Decks
{
    public class Cards
    {
        ...
        public static Card RandomByArena(Card.Rarity rarity, List<string> chestArenas)
        {
            ...
            {
                var card = new Card(int.Parse(exclusivecard.Split("-")[0]), int.Parse(exclusivecard.Split("-")[1]), false);
        ...
        public static Card Random(Card.Rarity rarity)
        {
            ...
            foreach (var exclusivecard in Resources.Configuration.exclusiveCards)
            {
                if (card == new Card(int.Parse(exclusivecard.Split("-")[0]), int.Parse(exclusivecard.Split("-")[1]), false))
                {
                    return null;
                }
            }
